...
import { Card, GameSettings, GameState, GameStateData } from "../types.js";

export function startGame(settings: GameSettings): GameStateData {
    return {
...
function generateShuffledCards(symbols: string[]): Card[] {
    let nextId: number = 0;
    const deck: Card[] = symbols.flatMap((symbol) => [
...
export function flipCard(state: GameStateData, cardId: number): void {
...
export function resolveMatch(state: GameStateData): void {
    if (state.flippedCardIds.length !== 2) { return; }
...
function checkGameEnd(state: GameStateData): boolean {
